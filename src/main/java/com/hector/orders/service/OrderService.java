...
public class OrderService {
    ...
    public Order createOrderItem(Long orderId, OrderItemsDTO dto) {
        ...
        if (order.getStatus() != OrderStatus.CREATED) {
            throw new RuntimeException("La orden no está en estado CREATED");
        }
    ...
    public Order confirmarOrder(Long orderId) {
        ...
        if (order.getStatus() != OrderStatus.CREATED) {
            throw new RuntimeException("Solo se pueden confirmar órdenes en estado CREATED");
        }
        ...
        if (order.getTotal() == null || order.getTotal().compareTo(BigDecimal.ZERO) <= 0) {
            throw new RuntimeException("El total debe ser mayor que 0");
        }
